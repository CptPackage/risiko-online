...
SET @PlayerB := 'player3';
SET @PlayerANation1 := (SELECT nation FROM Territory WHERE occupier = @PlayerA AND occupyingTanksNumber > 1 ORDER BY nation ASC LIMIT 1);
SET @PlayerANation2 = (SELECT nation FROM Territory WHERE occupier = @PlayerA AND occupyingTanksNumber > 1 ORDER BY nation DESC LIMIT 1);
SET @PlayerBNation1 = (SELECT nation FROM Territory WHERE occupier = @PlayerB AND occupyingTanksNumber = 1 ORDER BY nation ASC LIMIT 1);
SET @PlayerBNation2 = (SELECT nation FROM Territory WHERE occupier = @PlayerB AND occupyingTanksNumber = 1 ORDER BY nation DESC LIMIT 1);
