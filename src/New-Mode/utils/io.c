...
void print_ordered_list(const char items[], size_t length, bool padded)
{
	for (int itemNumber = 1; itemNumber <= length; itemNumber++)
	{
		if (padded)
		{
			printf("\t%d) %s\n", itemNumber, items[itemNumber - 1])
		}
		else
		{
			printf("%d) %s\n", itemNumber, items[itemNumber - 1])
...
void print_list(const char items[], size_t length, bool padded)
{
	for (int itemNumber = 1; itemNumber <= length; itemNumber++)
	{
		if (padded)
